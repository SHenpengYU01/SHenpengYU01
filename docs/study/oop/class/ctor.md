...
!!! info "移动控制"
    - 只有一个类没有定义任何自己版本的拷贝控制成员，且类的每个非static数据成员都可以移动时，编译器才会为它生成默认的移动构造函数或移动赋值运算符。内置类型成员可移动。  
    - 定义了一个移动构造函数或移动赋值运算符的类必须定义自己的拷贝操作，否则这些成员默认地被定义为删除的。
