...
!!! info "消解规则"
    给定两个子句，推出一个新子句：从子句 𝐷1 ∪ {𝐿} 和 𝐷2 ∪ {$\bar{𝐿}$} 推出子句 𝐷1 ∪ 𝐷2。其中，𝐷1 和 𝐷2 可为空集。𝐷1 ∪ 𝐷2 称为输入子句关于 𝐿 的消解式。其中，𝐿 称为 𝐿 的补。对于任意原子命题 $p: \bar{p}=\neg p, \bar{\bar{p}}=p$
...
SAT 可以被描述为一个约束满足问题。它从一组子句集合出发，寻找一个可以满足这些子句的真假赋值。例如，对于公式 (𝑝 ∨ 𝑞) ∧ (¬𝑝 ∨ 𝑟)，可以找到一个真假赋值 𝑣 满足这个公式.

<img src="\img\study\ai\ailogic\sat.png" alt="DPLL算法">
